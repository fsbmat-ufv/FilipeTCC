...
ptM <- dados %>% filter(Ano=="2009") %>% group_by(Mandante) %>% summarise(PontMan=sum(PontMandante))
...
ptV <- dados %>% filter(Ano=="2009") %>% group_by(Visitante) %>% summarise(PontVis=sum(PontVisitante))
ptV %>%  ggplot(aes(PontVis,reorder(Visitante,PontVis),fill=PontVis))+
  ...
  ylab("Times que disputaram o Brasileirão")+
  geom_text(aes(label=PontVis),nudge_x = 0.75)+
  theme_bw()+
...
totJogos %>%  ggplot(aes(Total,reorder(Time,Total),fill=Total))+
  ...
  ylab("Times que disputaram o Brasileirão")+
  theme_bw()+
  geom_text(aes(label=Total),nudge_x = 12)+
